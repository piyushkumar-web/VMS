...
// // async function sendApprovalMail(pass, verifyUrl) {
// //   // Method 1: Gmail SMTP (Works with any email recipient)
// //   if (process.env.MAIL_USER && process.env.MAIL_PASS && !process.env.MAIL_USER.includes('your_gmail')) {
// //     try {
// //       const transporter = nodemailer.createTransport({
// //         host: 'smtp.gmail.com',
// //         port: 587,
...
// //       const info = await transporter.sendMail({
// //         from: `"VMS - Grazitti Interactive" <${process.env.MAIL_USER}>`,
// //         to: pass.email,
// //         subject: 'Your Access Pass Has Been Approved - Set Your Password',
// //         html: `
// //           <div style="font-family:sans-serif;max-width:520px;margin:auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0">
...
// //               <p style="color:#64748b;margin:0 0 20px">Hi <strong>${pass.fullName}</strong>, your access pass request has been approved.</p>
// //               <table style="width:100%;border-collapse:collapse;margin-bottom:24px">
// //                 <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Pass ID</td><td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.passId || 'N/A'}</td></tr>
// //                 <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Valid From</td><td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.startDate || 'N/A'}</td></tr>
// //                 <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Valid Until</td><td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.endDate || 'N/A'}</td></tr>
// //                 <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Purpose</td><td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.purpose || 'N/A'}</td></tr>
// //               </table>
// //               <a href="${verifyUrl}" style="display:inline-block;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;font-weight:700;padding:14px 32px;border-radius:12px;text-decoration:none;font-size:15px">
// //                 Verify and Set Password
// //               </a>
...
// // // ================= EMAIL FUNCTION =================
// // async function sendApprovalMail(pass, verifyUrl) {
// //   // Method 1: Resend (Working - uses HTTPS)
// //   if (process.env.RESEND_API_KEY && resend) {
// //     try {
// //       const { data, error } = await resend.emails.send({
// //         from: process.env.FROM_EMAIL || 'onboarding@resend.dev',
...
// //   // Method 2: Console log (fallback for development)
// //   console.log('='.repeat(60));
// //   console.log('📧 EMAIL WOULD BE SENT:');
// //   console.log(`To: ${pass.email}`);
// //   console.log(`Subject: Your Access Pass Has Been Approved - Set Your Password`);
// //   console.log(`Link: ${verifyUrl}`);
// //   console.log('='.repeat(60));
...
// //       return res.json({ 
// //         success: true, 
// //         message: 'No email service configured. Emails will be logged to console.',
// //         mode: 'development'
// //       });
// //     }
...
// //     const fakePass = { 
// //       fullName: 'Test User', 
// //       email: process.env.TEST_EMAIL || process.env.FROM_EMAIL?.split('<')[1]?.replace('>', '') || process.env.MAIL_USER || 'test@example.com',
// //       passId: 'TEST-001',
// //       startDate: new Date().toISOString().split('T')[0],
// //       endDate: new Date().toISOString().split('T')[0],
// //       purpose: 'Mail Config Test'
...
// //     const testUrl = `${process.env.CLIENT_URL}/pass/verify/test-token`;
// //     const result = await sendApprovalMail(fakePass, testUrl);
    
// //     res.json({ 
// //       success: true, 
// //       message: `Test email sent to ${fakePass.email}`,
...
// //       success: false, 
// //       error: err.message,
// //       fix: 'Get a free API key from https://resend.com'
// //     });
// //   }
// // });

// // // 📝 REGISTER - Submit pass request
// // router.post('/register', async (req, res) => {
// //   try {
// //     const { geoLocation, ...rest } = req.body;
// //     const body = { ...rest };
...
// //       body.geoLocation = { 
// //         ...geoLocation, 
// //         isInsidePremises: dist <= OFFICE.radiusMeters, 
// //         lastVerified: new Date() 
// //       };
// //     }
    
// //     // Check if email already exists
// //     const existingPass = await Pass.findOne({ email: rest.email?.toLowerCase() });
// //     if (existingPass) {
...
// // // 📋 GET ALL PASSES (ADMIN with filters + pagination)
// // router.get('/', protect, adminOnly, async (req, res) => {
// //   try {
// //     const { status, search, page = 1, limit = 15, startDate, endDate } = req.query;
// //     const query = {};
...
// // // 📊 GET ALL ATTENDANCE LOGS (ADMIN with pagination + filters)
// // router.get('/attendance', protect, adminOnly, async (req, res) => {
// //   try {
// //     const { search, date, status, page = 1, limit = 15 } = req.query;
// //     const passQuery = {};
...
// //     const pass = await Pass.findOne({ email: email.trim().toLowerCase() }) || 
// //                  await Pass.findOne({ email: email.trim() });
    
// //     if (!pass) return res.status(401).json({ message: 'No pass found for this email address' });
// //     if (pass.status !== 'approved') {
// //       return res.status(401).json({ message: `Your pass is ${pass.status}. Only approved passes can log in.` });
// //     }
// //     if (!pass.isVerified) {
// //       return res.status(401).json({ message: 'Email not verified yet. Please check your email and click the verification link.' });
// //     }
// //     if (!pass.passwordHash) {
// //       return res.status(401).json({ message: 'Password not set. Please use the link sent to your email.' });
// //     }
...
// //       return res.status(403).json({ message: `Your pass expired on ${pass.endDate}` });
// //     }
    
// //     const token = jwt.sign(
// //       { passId: pass._id, type: 'pass' }, 
...
// // // 👤 GET CURRENT PASS USER INFO
// // router.get('/me', passAuth, async (req, res) => {
// //   res.json(req.pass);
// // });
...
// //         message: `You must be inside office premises to check in. You are ${Math.round(dist)}m away.` 
// //       });
// //     }
...
// //     if (today < pass.startDate || today > pass.endDate) {
// //       return res.status(403).json({ message: 'Your pass is not valid today' });
// //     }
...
// //     pass.attendanceLogs.push({ 
// //       date: today, 
// //       checkIn: new Date(), 
// //       checkInGeo: { lat, lng, isInsidePremises } 
// //     });
// //     pass.geoLocation = { lat, lng, accuracy, lastVerified: new Date(), isInsidePremises };
// //     await pass.save();
...
// //       durationMins: Math.round((log.checkOut - log.checkIn) / 60000) 
// //     });
// //   } catch (err) {
...
// //     const pass = await Pass.findOne({ email: email.trim() }) || 
// //                  await Pass.findOne({ email: email.trim().toLowerCase() });
    
// //     if (!pass) return res.status(404).json({ message: 'No pass found for this email address' });
// //     if (pass.status !== 'approved') {
// //       return res.status(403).json({ message: `Your pass is ${pass.status} and cannot be accessed` });
// //     }
...
// //     res.json(pass);
// //   } catch (err) {
// //     console.error('Tenure update error:', err);
// //     res.status(500).json({ message: err.message });
// //   }
// // });
...
// // router.get('/debug/:email', async (req, res) => {
// //   try {
// //     const pass = await Pass.findOne({ email: req.params.email })
// //       .select('email status isVerified passwordHash startDate endDate fullName passId');
...
// // Force IPv4 globally - CRITICAL for SMTP
// dns.setDefaultResultOrder('ipv4first');

// // ================= CONFIG =================
// const OFFICE = { lat: 30.6942, lng: 76.8606, radiusMeters: 200 };

// // Initialize Resend (fallback only)
// let resend = null;
// if (process.env.RESEND_API_KEY) {
//   resend = new Resend(process.env.RESEND_API_KEY);
//   console.log('✅ Resend initialized (fallback)');
// }
...
// // ================= EMAIL FUNCTION - GMAIL SMTP PRIMARY =================
// async function sendApprovalMail(pass, verifyUrl) {
//   // METHOD 1: Gmail SMTP (Primary - Works with any email recipient)
//   const gmailUser = process.env.MAIL_USER;
//   const gmailPass = process.env.MAIL_PASS;
  
//   if (gmailUser && gmailPass && !gmailUser.includes('your_gmail')) {
//     try {
//       console.log('📧 [GMAIL] Attempting to send via Gmail SMTP...');
...
//       const mailOptions = {
//         from: `"VMS - Grazitti Interactive" <${gmailUser}>`,
//         to: pass.email,
//         subject: 'Your Access Pass Has Been Approved - Set Your Password',
//         html: `
//           <div style="font-family:sans-serif;max-width:520px;margin:auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e2e8f0">
...
//                 <tr>
//                   <td style="padding:8px 0;color:#94a3b8;font-size:13px">Pass ID</td>
//                   <td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.passId || 'N/A'}</td>
//                 </tr>
//                 <tr>
//                   <td style="padding:8px 0;color:#94a3b8;font-size:13px">Valid From</td>
//                   <td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.startDate || 'N/A'}</td>
//                 </tr>
//                 <tr>
//                   <td style="padding:8px 0;color:#94a3b8;font-size:13px">Valid Until</td>
//                   <td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.endDate || 'N/A'}</td>
//                 </tr>
//                 <tr>
//                   <td style="padding:8px 0;color:#94a3b8;font-size:13px">Purpose</td>
//                   <td style="padding:8px 0;font-weight:600;color:#1e293b;font-size:13px">${pass.purpose || 'N/A'}</td>
//                 </tr>
//               </table>
//               <a href="${verifyUrl}" style="display:inline-block;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;font-weight:700;padding:14px 32px;border-radius:12px;text-decoration:none;font-size:15px">
//                 Verify and Set Password
...
//       console.log('✅ [GMAIL] Mail sent successfully! Message ID:', info.messageId);
//       return { success: true, provider: 'Gmail', id: info.messageId };
      
//     } catch (err) {
//       console.error('❌ [GMAIL] Failed:', err.message);
//       if (err.message.includes('Invalid login') || err.message.includes('535')) {
//         console.error('💡 Gmail login failed. Make sure you\'re using an App Password, not your regular Gmail password.');
//         console.error('   Generate one at: https://myaccount.google.com/apppasswords');
//       }
//       if (err.message.includes('ETIMEDOUT') || err.message.includes('ENETUNREACH')) {
//         console.error('💡 Network is blocking SMTP ports. Will try fallback methods...');
//       }
//       // Don't throw - try fallback
//     }
//   } else {
//     console.log('⚠️ [GMAIL] No valid Gmail credentials found in .env (MAIL_USER/MAIL_PASS)');
//   }
  
//   // METHOD 2: Resend (Fallback - works over HTTPS)
//   if (process.env.RESEND_API_KEY && resend) {
//     try {
//       console.log('📧 [RESEND] Attempting fallback via Resend...');
...
//       if (err.message.includes('only send testing emails')) {
//         console.error('💡 Resend domain verification required. Add a verified domain at resend.com/domains');
//       }
//       // Don't throw - try console fallback
//     }
//   }
  
//   // METHOD 3: Console fallback (Development only)
//   console.log('='.repeat(70));
//   console.log('📧 [CONSOLE] NO EMAIL SENT - VERIFICATION LINK (Manual Sharing Required):');
//   console.log(`To: ${pass.email}`);
//   console.log(`Link: ${verifyUrl}`);
//   console.log('='.repeat(70));
//   console.log('💡 To enable email sending:');
//   console.log('   1. Gmail: Add MAIL_USER and MAIL_PASS (App Password) to .env');
//   console.log('   2. Resend: Add RESEND_API_KEY to .env');
//   console.log('='.repeat(70));
  
//   return { success: true, provider: 'Console', message: 'Email logged to console (no email sent)' };
// }
...
//       startDate: new Date().toISOString().split('T')[0],
//       endDate: new Date().toISOString().split('T')[0],
//       purpose: 'Mail Configuration Test'
//     };
...
//     res.json({ 
//       success: true, 
//       message: `Test ${result.provider === 'Console' ? 'logged to console' : 'email sent'} to ${testEmail}`,
//       provider: result.provider,
//       ...(result.provider === 'Console' && { manualLink: testUrl })
//     });
//   } catch (err) {
//     console.error('TEST MAIL ERROR:', err);
//     res.status(500).json({ 
//       success: false, 
//       error: err.message,
//       fix: 'Check MAIL_USER/MAIL_PASS in .env (use Gmail App Password) or add RESEND_API_KEY'
//     });
//   }
// });

// // 🔍 DEBUG: Test Gmail SMTP connection
// router.get('/test-smtp', protect, adminOnly, async (req, res) => {
//   try {
//     const transporter = nodemailer.createTransport({
//       host: 'smtp.gmail.com',
//       port: 587,
...
//     try {
//       const result = await sendApprovalMail(pass, verifyUrl);
//       res.json({ 
//         ...pass.toObject(), 
//         mailSent: result.provider !== 'Console',
//         provider: result.provider,
//         ...(result.provider === 'Console' && { manualLink: verifyUrl })
//       });
//     } catch (mailErr) {
//       console.error('[MAIL ERROR]', mailErr.message);
//       res.json({ 
...
// const jwt = require('jsonwebtoken');
// const crypto = require('crypto');
// const nodemailer = require('nodemailer'); // Only declared once at top
// const { Resend } = require('resend');
// const dns = require('dns');
...
// // ================= EMAIL FUNCTION - CONSOLE PRIMARY (Since SMTP is blocked) =================
// async function sendApprovalMail(pass, verifyUrl) {
//   // Since Gmail SMTP is blocked by your network (IPv6 issues), use console logging
//   // This is the most reliable method for your current network situation
  
//   console.log('='.repeat(70));
//   console.log('📧 [VMS] PASS APPROVAL - VERIFICATION LINK');
//   console.log(`To: ${pass.email}`);
//   console.log(`Name: ${pass.fullName}`);
//   console.log(`Link: ${verifyUrl}`);
//   console.log('='.repeat(70));
//   console.log('📌 SHARE THIS LINK WITH THE USER MANUALLY');
//   console.log('='.repeat(70));
  
//   // Try Resend as fallback (only works with verified domain or own email)
//   if (process.env.RESEND_API_KEY && resend) {
//     try {
//       console.log('📧 [RESEND] Attempting fallback...');
...
//   return { success: true, provider: 'Console', message: 'Link logged to console', manualLink: verifyUrl };
// }
...
//     res.json({ 
//       success: true, 
//       message: result.provider === 'Console' 
//         ? `Verification link logged to console. Share manually with ${testEmail}`
//         : `Email sent to ${testEmail}`,
//       provider: result.provider,
//       ...(result.manualLink && { manualLink: result.manualLink })
//     });
//   } catch (err) {
//     console.error('TEST MAIL ERROR:', err);
//     res.status(500).json({ 
